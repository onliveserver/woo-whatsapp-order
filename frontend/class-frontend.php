...
if (! class_exists('Onlive_WA_Order_Pro_Frontend')) {
	class Onlive_WA_Order_Pro_Frontend
	{
		...
		public function __construct(Onlive_WA_Order_Pro $plugin)
		{
			...
			// Register on admin_init as backup
		...
		public function enqueue_assets()
		{
			...
			$localized_data = [
				...
				'template'  => [
					'enabled'   => (bool) $this->plugin->get_setting('template_enabled', false),
					'custom'    => $this->plugin->get_setting('message_template', ''),
					'includeLink' => (bool) $this->plugin->get_setting('include_product_link', true),
				],
				'strings'   => [
...
<?php
		...
		public function handle_ajax_message()
		{
			...
			try {
				...
				if (empty($url)) {
					$debug_info['error'] = 'Phone number missing in plugin settings.';
					$this->send_json_response(false, 'Please add your WhatsApp number in the plugin settings.', ['debug' => $debug_info]);
				}
