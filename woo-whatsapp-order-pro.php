...
if ( ! class_exists( 'Onlive_WA_Order_Pro' ) ) {
	final class Onlive_WA_Order_Pro {
		 ...
		 *
		 * @var Onlive_WA_Order_Frontend|null
		 */
		 ...
		 *
		 * @var Onlive_WA_Order_Ajax_Handler|null
		 */
	...
	private function includes() {
		require_once ONLIVE_WA_ORDER_PATH . 'includes/class-settings-index.php';
		require_once ONLIVE_WA_ORDER_PATH . 'includes/class-template-parser.php';
		require_once ONLIVE_WA_ORDER_PATH . 'includes/class-product-data.php';
		require_once ONLIVE_WA_ORDER_PATH . 'includes/class-product-link.php';
		require_once ONLIVE_WA_ORDER_PATH . 'includes/class-frontend.php';
		require_once ONLIVE_WA_ORDER_PATH . 'includes/class-ajax-handler.php';
                require_once ONLIVE_WA_ORDER_PATH . 'includes/class-unified-ajax.php';
		require_once ONLIVE_WA_ORDER_PATH . 'includes/class-data-preparer.php';
		require_once ONLIVE_WA_ORDER_PATH . 'includes/class-formatter.php';
	...
	public function bootstrap() {
		...
		// Also exclude on WooCommerce checkout AJAX requests
		...
		$this->frontend = new Onlive_WA_Order_Frontend( $this );
		$this->ajax_handler = new Onlive_WA_Order_Ajax_Handler( $this );

		// Initialize unified AJAX handler to replace multiple redundant handlers
		new Onlive_WA_Order_Unified_Ajax( $this );

		// Add custom AJAX endpoint to bypass admin-ajax.php blocking
		add_action( 'init', [ $this, 'register_custom_ajax_endpoint' ] );
		add_action( 'template_redirect', [ $this, 'handle_custom_ajax_request' ] );

	}

	/**
	 * Register custom AJAX endpoint to bypass admin-ajax.php blocking.
	 */
	public function register_custom_ajax_endpoint() {
		add_rewrite_rule( '^wa-ajax/?$', 'index.php?wa_ajax=1', 'top' );
		add_rewrite_tag( '%wa_ajax%', '1' );
	}

	/**
	 * Handle custom AJAX endpoint requests.
	 */
	public function handle_custom_ajax_request() {
		if ( ! get_query_var( 'wa_ajax' ) ) {
			return;
		}
		...
		if ( $action === 'vaog2jucg3f2' ) {
			if ( $this->ajax_handler && method_exists( $this->ajax_handler, 'handle_ajax_message' ) ) {
				$this->ajax_handler->handle_ajax_message();
			} elseif ( class_exists( 'Onlive_WA_Order_Unified_Ajax' ) ) {
				// If unified AJAX class exists, call its entry point
				$unified = new Onlive_WA_Order_Unified_Ajax( $this );
				$unified->handle_ajax();
			}
		} elseif ( $action === 'onlive_wa_ping' ) {
			if ( $this->ajax_handler && method_exists( $this->ajax_handler, 'handle_ping' ) ) {
				$this->ajax_handler->handle_ping();
			} elseif ( class_exists( 'Onlive_WA_Order_Unified_Ajax' ) ) {
				$unified = new Onlive_WA_Order_Unified_Ajax( $this );
				$unified->handle_ajax();
			}
		...
		public function is_enabled() {
			return (bool) Onlive_WA_Order_Settings_Index::get( 'enabled', true );
		}
		...
		public function should_render_button( $context ) {
			$positions = Onlive_WA_Order_Settings_Index::get( 'positions', [] );
		...
		public function generate_message( $context, $data = [] ) {
			$template = '';
			if ( Onlive_WA_Order_Settings_Index::get( 'template_enabled', false ) ) {
				$template = (string) Onlive_WA_Order_Settings_Index::get( 'message_template', '' );
			}
		...
		public function get_whatsapp_url( $message ) {
			$raw_phone = (string) Onlive_WA_Order_Settings_Index::get( 'phone', '+919100454045' );
